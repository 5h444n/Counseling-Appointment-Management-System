    ...
    <nav class="flex-1 px-3 py-6 space-y-2 overflow-y-auto">
        ...
        @if(Auth::user()->role === 'advisor')
            <a href="{{ route('advisor.slots') }}"
               class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200
           {{ request()->routeIs('advisor.slots') ? 'bg-orange-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
        ...
        @if(Auth::user()->role === 'student')
            <a href="#" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200 text-gray-300 hover:bg-slate-800 hover:text-white">
